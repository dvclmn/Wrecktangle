...
public extension SwiftBox.PartPreset {
  ...
  //        return "\(left)\(middle)\(right)"
  //
  //      case (.threeByTwo, .header), (.threeByTwo, .divider):
  //        let left = self.content[1, 0]
  //        let middle = String(repeating: " ", count: width - 2)
  //        let right = self.content[1, 2]
  //        return "\(left)\(middle)\(right)"
  //
  //      case (.threeByTwo, .content):
  //        let left = self.content[1, 0]
  //        let middle = String(repeating: " ", count: width - 2)
  //        let right = self.content[1, 2]
  //        return "\(left)\(middle)\(right)"
...
public extension SwiftBox {
  ...
  struct BoxBuilder {
    ...
    private func partPreset(for part: BoxGlyph, with resolution: PartPreset.Resolution) -> PartPreset {
      switch resolution {
        case .oneByOne:
          return oneByOnePreset(for: part)
        case .threeByTwo:
    ...
    private func oneByOnePreset(for part: BoxGlyph) -> PartPreset {
      let char = theme.glyphSet.character(for: part)
      var grid = CharacterGrid(rows: 1, columns: 1)
      grid[0, 0] = char
      return PartPreset(content: grid, resolution: .oneByOne)
    }
    ...
    private func threeByTwoPreset(for part: BoxGlyph) -> PartPreset {
      ...
      switch part {
        case .corner(.topLeading):
          grid[0, 0] = theme.glyphSet.character(for: .corner(.topLeading))
          grid[0, 1] = theme.glyphSet.character(for: .horizontal(.exterior))
          grid[0, 2] = theme.glyphSet.character(for: .horizontal(.exterior))
          grid[1, 0] = theme.glyphSet.character(for: .vertical(.exterior))
          grid[1, 1] = " "
          grid[1, 2] = theme.glyphSet.character(for: .corner(.topLeading))
